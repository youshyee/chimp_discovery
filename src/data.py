...
def readFlow(sample_dir, resolution, to_rgb):
    ...
    h, w, _ = np.shape(flow)
    flow = cv2.resize(
        flow, (resolution[1], resolution[0]), interpolation=cv2.INTER_LINEAR)
...
def readRGB(sample_dir, resolution):
    ...
    rgb = cv2.resize(
        rgb, (resolution[1], resolution[0]), interpolation=cv2.INTER_LINEAR)
    rgb = np.clip(rgb, -1., 1.)
    ...
    def __getitem__(self, idx):
        ...
        for flo in flos:
            ...
            rgb_dir = os.path.join(
                self.data_dir[1], flosplit[-2], flosplit[-1]).replace('.flo', '.jpg')
            gt_dir = os.path.join(
                self.data_dir[2], flosplit[-2], flosplit[-1]).replace('.flo', '.png')
        ...
        out = np.stack(flows, 0) if not self.with_rgb else np.stack(
            [np.stack(flows, 0), np.stack(rgbs, 0)], -1)
...
class FlowEval(Dataset):
    def __init__(self, data_dir, resolution, pair_list, val_seq, to_rgb=False, with_rgb=False):
        ...
        self.samples = [os.path.join(
            x.split('/')[-2], x.split('/')[-1]) for x in self.samples]
    ...
    def __getitem__(self, idx):
        ...
        for gap, _gap in zip(self.gaps, self.neg_gaps):
            flow_dir = os.path.join(self.data_dir[0], self.samples[idx]).replace(
                'gap1', gap).replace('.jpg', '.flo')
            if os.path.exists(flow_dir):
                out.append(readFlow(flow_dir, self.resolution, self.to_rgb))
                fgap.append(gap)
            else:
                flow_dir = os.path.join(self.data_dir[0], self.samples[idx]).replace(
                    'gap1', _gap).replace('.jpg', '.flo')
                out.append(readFlow(flow_dir, self.resolution, self.to_rgb))
                fgap.append(_gap)
        out = np.stack(out, 0)
        ...
        gt_dir = os.path.join(
            self.data_dir[2], self.samples[idx]).replace('.jpg', '.png')
        img_dir = gt_dir.split('/')[-2:]
