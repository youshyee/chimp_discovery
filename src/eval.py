...
def eval_unsupervised(val_loader, model, device, moca, use_flow, it, resultsPath=None, writer=None, train=False):
    # for unsupervised only visualize output
    with torch.no_grad():
        t = time.time()
        ...
        for idx, val_sample in enumerate(val_loader):
            ...
            if train:
                # save
                grid = ut.convert_for_vis(
                    flows.unsqueeze(1), use_flow=use_flow)
                grid_ri = ut.convert_for_vis(
                    recon_image, use_flow=use_flow).unsqueeze(1)
                grid_r = ut.convert_for_vis(recons, use_flow=use_flow)
                grid_m = (
                    255*torch.cat([masks, masks, masks], dim=3)).type(torch.ByteTensor)
                grid_m = einops.rearrange(
                    grid_m, 'b t s c h w -> (b t) s c h w')
                grid_all = torch.cat(
                    [grid, grid_ri, grid_m, grid_r], dim=1)
                nrow = grid_all.size()[1]
                grid_all = einops.rearrange(
                    grid_all, 'b s c h w -> (b s) c h w')
                grid_all = torchvision.utils.make_grid(grid_all, nrow=nrow)
                outpath = os.path.join(resultsPath, f'{it+idx}.jpg')
                torchvision.io.write_jpeg(grid_all, outpath, quality=100)
    ...
    with torch.no_grad():
        ...
        for idx, val_sample in enumerate(val_loader):
            ...
            masks = einops.rearrange(
                masks, '(b t) s c h w -> b t s c h w', t=4)
            for i in range(masks.size()[0]):
                save_masks, mean_mask, iou, single_step_iou = ut.ensemble_hungarian_iou(
                    masks[i], gt[i:i+1], moca)  # t 1 h w
                ...
                if not train:
                    ...
                    for mi, save_mask in enumerate(save_masks):
                        fgp = fgap[mi][0]
                        save_mask = einops.rearrange(
                            save_mask, 'c h w -> h w c')
                        os.makedirs(os.path.join(
                            resultsPath, category, fgp), exist_ok=True)
                        cv2.imwrite(os.path.join(
                            resultsPath, category, fgp, index), (save_mask * 255.))
                    mean_mask = einops.rearrange(mean_mask, 'c h w -> h w c')
                    os.makedirs(os.path.join(
                        resultsPath, category, 'mean'), exist_ok=True)
                    cv2.imwrite(os.path.join(resultsPath, category,
                                'mean', index), (mean_mask * 255.))
        ...
        frame_single_step_mean_iou = sum(
            single_step_frameious) / len(single_step_frameious)
        print(' --> inference, time {}'.format(time.time()-t),
        ...
        if train:
            ...
            writer.add_scalar('IOU/val_single_step_mean',
                              frame_single_step_mean_iou, it)
...
def main(args):
    ...
    print('======> start inference {}, {}, use {}.'.format(
        args.dataset, args.verbose, device))
    # inference / evaluate on validation set
    eval(val_loader, model, device, moca, use_flow,
         it, resultsPath=resultsPath, train=False)
